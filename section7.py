...
class section7GUI(customtkinter.CTk):
    def __init__(self):
        ...
        label = customtkinter.CTkLabel(
            master=frame, text="Section 7:", font=("Roboto", 24))
        ...
        enterSeqLabel = customtkinter.CTkLabel(
            master=frame, text="Enter Sequence For Indexing:", font=("Roboto", 16)).pack()
        SeqEntry = customtkinter.CTkEntry(
            master=frame, textvariable=seqVar, placeholder_text="Sequence").pack(padx=12, pady=12)

        indexingFunctionPartial = partial(
            indexingFunction, seqVar, resultLabel)
        indexingbtn = customtkinter.CTkButton(
            master=frame, text="Get Index!", command=indexingFunctionPartial).pack(padx=12, pady=12)
