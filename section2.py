...
class section2GUI(customtkinter.CTk):
    def __init__(self):
        ...
        Reverse_Complement_part = partial(
            Reverse_Complement, newSeq, resultLabel)

        # function -> button -> label
        GC_btn = customtkinter.CTkButton(
            master=frame, text="Get GC Content!", command=GC_Content_part).pack(padx=12, pady=12)
        Complement_btn = customtkinter.CTkButton(
            master=frame, text="Get Complement!", command=Complement_part).pack(padx=12, pady=12)
        Reverse_btn = customtkinter.CTkButton(
            master=frame, text="Get Reverse!", command=Reverse_part).pack(padx=12, pady=12)
        Reverse_Complement_btn = customtkinter.CTkButton(
            master=frame, text="Get Reverse Complement!", command=Reverse_Complement_part).pack(padx=12, pady=12)
        resultLabel.pack()
